...
def _import_app():
    # Allow running from repo root (`python -m server.app.ai_cli`) or from `server/` (`python -m app.ai_cli`)
    here = Path(__file__).resolve()  # .../server/app/ai_cli.py
    server_dir = here.parent.parent  # .../server
    repo_root = server_dir.parent    # .../FollowThru
...
def main() -> int:
    ...
    try:
        ...
        if "httpx" in str(exc).lower():
            ...
            print("Install with: pip install httpx  (or pip install -r server/requirements.txt)", file=sys.stderr)
            return 2
        raise
